...
class ServiceListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func fetchData() {
        ...
        URLSession.shared.dataTask(with: url) { data, _, _ in
            if let data = data, let decoded = try? JSONDecoder().decode([Service].self, from: data) {
...
class AddServiceViewController: UIViewController {
    ...
    @IBAction func saveBtnPressed(_ sender: Any) {
        let newService = Service(service_id: nil, service_code: txtCode.text ?? "", name: txtName.text ?? "", price: txtPrice.text ?? "0", availability: true, description: txtDesc.text ?? "")
...
class EditServiceViewController: UIViewController {
    ...
    @IBAction func saveBtnPressed(_ sender: Any) {
        ...
        let updated = Service(service_id: id, service_code: txtCode.text ?? s.service_code, name: txtName.text ?? s.name, price: txtPrice.text ?? s.price, availability: s.availability, description: txtDesc.text ?? s.description)
