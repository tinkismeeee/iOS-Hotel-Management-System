...
// MARK: - 2. CUSTOM CELL
class RoomCellAdmin: UITableViewCell {
...
class RoomListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func fetchData() {
        ...
        URLSession.shared.dataTask(with: url) { data, _, _ in
            if let data = data, let decoded = try? JSONDecoder().decode([RoomAdmin].self, from: data) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "RoomCellAdmin", for: indexPath) as! RoomCellAdmin
...
class AddRoomViewController: UIViewController {
    ...
    @IBAction func saveBtnPressed(_ sender: Any) {
        ...
        let newRoom = RoomAdmin(room_id: nil, room_number: txtNumber.text ?? "", room_type_id: 1, floor: floor, price_per_night: txtPrice.text ?? "0", max_guests: maxGuests, bed_count: bedCount, description: txtDesc.text ?? "", status: txtStatus.text ?? "available", is_active: true, room_type_name: txtTypeName.text ?? "")
...
class EditRoomViewController: UIViewController {
    ...
    @IBAction func saveBtnPressed(_ sender: Any) {
        ...
        let updatedRoom = RoomAdmin(room_id: id, room_number: txtNumber.text ?? "", room_type_id: r.room_type_id, floor: floor, price_per_night: txtPrice.text ?? "0", max_guests: maxGuests, bed_count: bedCount, description: txtDesc.text ?? "", status: txtStatus.text ?? r.status, is_active: r.is_active, room_type_name: txtTypeName.text ?? "")
